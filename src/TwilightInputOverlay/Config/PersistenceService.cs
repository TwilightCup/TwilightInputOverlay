...
namespace TwilightInputOverlay
{
    ...
    /// error reporting (report the offending line's number and file).
    /// </summary>
    ...
    /// <summary>
    /// Line-oriented, sectioned key=value reader/writer for all
    /// TwilightInputOverlay config files, following the same format and
    /// tolerant parsing rules as HSRTimer. Format:
    /// <code>
    /// # line comments
    /// [section]
    /// key = value
    ...
    /// line number; the rest of the file still loads.
    /// </summary>
    public static class PersistenceService
    {
        ...
        public static IEnumerable<ParsedLine> Read(string filePath)
        {
            ...
            {
                Plugin.Logger.LogWarning($"TwilightInputOverlay: failed to read {shortName}: {ex.Message}");
                return result;
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (eq <= 0)
                {
                    Plugin.Logger.LogWarning($"TwilightInputOverlay: {shortName}({lineNo}): expected 'key = value', skipping line.");
                    continue;
                }
                ...
                if (key.Length == 0)
                {
                    Plugin.Logger.LogWarning($"TwilightInputOverlay: {shortName}({lineNo}): empty key, skipping line.");
                    continue;
                }
        ...
        public static void Write(string filePath, IEnumerable<KeyValuePair<string, IDictionary<string, string>>> sections, string headerComment = null)
        {
            ...
            {
                Plugin.Logger.LogWarning($"TwilightInputOverlay: failed to write {System.IO.Path.GetFileName(filePath)}: {ex.Message}");
            }
        }
        ...
        public static void EnsureDirs()
        {
            ...
            {
                Plugin.Logger.LogWarning($"TwilightInputOverlay: failed to create config dirs: {ex.Message}");
            }
        }
    }
}
