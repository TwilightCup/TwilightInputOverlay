...
namespace TwilightInputOverlay
{
    ...
    /// <summary>
    /// Parser for one localization file, following the HSRTimer spec (R7.3).
    /// Format per line: <c>KEY:Translation</c>. '#' lines are comments; blank
    /// lines ignored; the value is everything after the first ':'; a literal
    /// '\n' escape becomes a newline; keys must match <c>^[A-Z][A-Z0-9_]*$</c>,
    /// except the special <c>__LANG_NAME__</c> key. UTF-8 (no BOM) assumed;
    /// '\n' and '\r\n' line endings both tolerated. Malformed lines are skipped
    /// with a logged warning naming the file and line number.
    /// </summary>
    public static class LanguageFile
    {
        ...
        public static IEnumerable<LangEntry> ParseLines(IList<string> lines, string sourceName, out string displayName)
        {
            ...
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                if (colon <= 0)
                {
                    Plugin.Logger.LogWarning($"TwilightInputOverlay: {sourceName}({lineNo}): expected 'KEY:Translation', skipping.");
                    continue;
                }
                ...
                {
                    Plugin.Logger.LogWarning($"TwilightInputOverlay: {sourceName}({lineNo}): invalid key '{key}' (must match [A-Z][A-Z0-9_]*), skipping.");
                    continue;
                }
